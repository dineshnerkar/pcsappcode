...
module.exports = (dbClient, liveUsers) => {
  ...
  async function autoRecallFun() {
    const query = `documents/search/infoCardVaultChange?startDate=${getUTCDate(new Date())}T08:30:00`;
    // const query = `documents/search/infoCardVaultChange?startDate=2022-06-20T08:30:00`
    ...
    if (recalledDocs && recalledDocs.length > 0) {
      ...
      if (docs && docs.length > 0) {
        ...
        for (let i = 0; i < intersection.length; i++) {
          ...
          const docInfo = await httpReq.get(getDocQ, apiKey);
  ...
  const autoRecallJob = schedule.scheduleJob('* 9 * * * *', autoRecallFun);
