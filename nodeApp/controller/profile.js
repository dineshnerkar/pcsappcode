...
module.exports = (dbClient, passport) => {
        ...
        if (!req.error) {
            if (req.body.name && req.files) {
                ...
                if (result && result.ops && result.ops[0]) {
                    fs.writeFile(`./templates/${result.ops[0]._id}.pdf`, req.files.data.data, (err) => {
                        if (err) {
                            req.error = err;
                            req.errorCode = 500;
                            next();
        ...
        if (!req.error) {
            ...
            if (req.body.name) {
                ...
                if (result && result.lastErrorObject && result.lastErrorObject.updatedExisting) {
                    if (req.files && req.files.data && req.body.fields) {
                        fs.writeFile(`./templates/${result.value._id}.pdf`, req.files.data.data, (err) => {
                            if (err) {
                                req.error = err;
                                req.errorCode = 500;
                                next();
    ...
    async function getFormFields(req, res, next) {
        ...
        fs.writeFile(`./formFiledTemplate/${filepath}.pdf`, req.files.data.data, async (err) => {
            if (err) {
                req.error = err;
                req.errorCode = 500;
                next();
                ...
                if (resData && !resData.success) {
                    fs.unlinkSync(`./formFiledTemplate/${filepath}.pdf`);
                    req.payload = resData;
                    next();
                } else {
                    fs.unlinkSync(`./formFiledTemplate/${filepath}.pdf`);
